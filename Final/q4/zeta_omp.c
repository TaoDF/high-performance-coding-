...
double Z (double t, int r) {    
    ...
    int k = 0;
    double R = 0.0;
    double tt = t/2.0 * log(t/2.0/M_PI) - t/2.0 - M_PI/8.0 + 1.0/48.0/t + 7.0/5760.0/t/t/t;
    ...
    for (int n = 1; n <= N_int; n++) {
		double temp_sum = (pow(n, -0.5) * cos(tt - (t * log(n))));;
        #pragma omp critical
		{
	     sum += temp_sum;
		}
	...
	#pragma omp parallel for
	for(int k_tem = k;k_tem<=r; ++k_tem)
	{
		double temp = C(k_tem,2.0*p-1.0) * pow(2.0 * piot, ((double) k_tem)*0.5); 
		#pragma omp critical
		{
			R+=temp;
		}
