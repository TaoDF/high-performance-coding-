...
//-------------------------
int sle_size = selections.size();

int thread_num = 4;
int eachIter = sle_size/thread_num;
int startIdx, endIdx;
...
for(int i = 0; i<thread_num;i++)
{
    ...
    threads1.push_back(std::thread([&, startIdx, endIdx] {

    for(int j=startIdx;j<endIdx;j++)
    {
        auto& individual_a = *selections[j].first;
        auto& individual_b = *selections[j].second;
        /* Perform the crossover and add the new individual to the new population */
        GATSP::individual ind = crossover(individual_a, individual_b);
...
//-----------------------
int ppl_size = population.size();
//#pragma omp parallel for

thread_num = 4;
eachIter = ppl_size/thread_num;
...
for(int i = 0; i<thread_num;i++)
{
    ...
    threads.push_back(std::thread([&, startIdx, endIdx] {
        ...
        for(int j=startIdx; j<endIdx; j++)
        {
            auto k = population[j].tour.begin();
            double distance = distances[first_index][*k];
            while (k != population[j].tour.end()) {
                auto& distances_i = distances.at(*k);
                ++k;
                if (k != population[j].tour.end()) {
                    distance += distances_i.at(*k);
                }
                else {
            ...
            population[j].metadata.distance = distance;
        }
    }));
}
...
GATSP::selection_container GATSP::selection()
{
    ...
    //--------------
    int thread_num = 4;
    int eachIter = kPopulationSize/thread_num;
    int startIdx, endIdx;
    std::vector<std::thread> threads;
    std::mutex mu;
